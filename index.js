...
function findMinMax([firstElement, ...restElements]) {
	...
	for (let i = 1; i < restElements.length; i++) {
		if (restElements[i] < minValue) {
			minValue = restElements[i];
		}
		if (restElements[i] > maxValue) {
			maxValue = restElements[i];
		}
	}
...
function swapMinMax(array) {
	...
	return [minValue, array[1], array[2], array[3], maxValue] = [maxValue, array[1], array[2], array[3], minValue];
}
...
function multiplyByIndex(arr) {
	...
	let newArr = [];
	for (const key in arr) {
		newArr[newArr.length] = arr[key] * key;
	}
	return newArr;
}
